...
- Next action: do not reopen UARTI/getty/stale-image questions unless new image or bootarg evidence contradicts this entry. Do not treat QEMU vmfd readiness as the primary suspect unless a future A/B contradicts `20260429-165235`. For post-kernel VM1 slowness, add rate-limited per-op/per-device QEMU/sel4 request classification for the `3s` to `61s` interval and inspect GICv3 vGIC registered-IRQ/maintenance behavior; current counters prove there are real forwarded requests but not which virtio device or MMIO operation dominates. Keep the VM1 allocator-pool/no-unity RAM/data/control mapping and cache-maintenance path as the leading pre-kernel suspect because VM1 VMM load remains much slower than VM0 and repeatedly shows `21.7 s` outliers. Treat seL4 kernel IPI slowness as low priority for this app unless affinity evidence changes: the generated image is SMP-capable, but VM0/VM1/VMM TCBs and one-vCPU guest TCBs are on affinity 0.
- Updated: 2026-04-29
