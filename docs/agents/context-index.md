...
- Last known state: clean Orin AGX rebuilds on 2026-04-29 confirmed VM1 UARTI routing on Autopilot `tty1`: VM1 generated `stdout=/bus@0/serial@31d0000` with `console=ttyAMA0,115200n8`. Request `20260429-132803` uses DT-selected `earlycon` rather than hard-coded `earlycon=pl011,...`; Linux still prints `earlycon: pl11 at MMIO32 0x31d0000` because `arm,sbsa-uart` earlycon is implemented by the PL011 driver, and the same log confirms `31d0000.serial: ttyAMA0 ... is a SBSA`. Autopilot did run `upload_driver_vm_rootfs` before EFI upload. The uploaded driver image contains `/var/lib/virt/images/user-vm.qcow2`, the embedded qcow2 matches the deploy user image by sha256 (`077f26787adcd700b50e7f289266dbe7bf337a76a9cbff88c8d5ad98106ea80c`), and embedded `/etc/inittab` contains `AMA0:12345:respawn:/usr/sbin/ttyrun ttyAMA0 /bin/start_getty 115200 ttyAMA0 vt102`. Therefore the missing VM1 login is not stale eMMC, skipped Autopilot deployment, or missing packaged `SERIAL_CONSOLES`/inittab. Timing run `20260429-145048` confirms a real VM1 pre-kernel VMM load problem: the same 44352000-byte `linux` image loaded in about `45.9 ms` for VM0 but about `21.7 s` for VM1, both with `clean_cache=1`. Runtime also shows VM0 RAM is `untyped memory (unity stage-2 mapping)` while VM1 RAM is `allocator pool (NO unity stage-2 mapping)`. Source review shows VM1 registers cross-VM async consume handlers before `load_vm_images()`, so VM0-origin notifications can theoretically contend during image load, but VM1 Linux is not started until after `load_vm_images()` and DTB dump; VM1 guest PCI probing cannot explain the measured pre-kernel load gap. `CONFIG_KMOD_SEL4_VIRT_STATS` now enables rate-limited `sel4_virt_stats:` counters to test the VM0-origin inter-VM/RPC CPU-burn suspect without restoring verbose per-event logs. Source-path review still records a second post-kernel slowness phase in the nested QEMU/seL4 device-model path.
- Next action: do not reopen UARTI/getty/stale-image questions unless new image or bootarg evidence contradicts this entry. For VM1 pre-kernel slowness, inspect the VM1 allocator-pool/no-unity RAM path used by `vm_ram_touch()` and per-page `seL4_ARM_Page_CleanInvalidate_Data()`; compare it to VM0's untyped/unity mapping path. Separately, run a clean Orin build with `CONFIG_VM_IMAGE_LOAD_TIMING=y` and `CONFIG_KMOD_SEL4_VIRT_STATS=y`, then correlate `VM_IMAGE_LOAD_TIMING` with `sel4_virt_stats:` counters for doorbells, handled IRQs, queued work, empty workqueue passes, forwarded RPCs, and poll-ready returns.
- Updated: 2026-04-29
