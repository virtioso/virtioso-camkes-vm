...
- Next action: do not reopen UARTI/getty/stale-image questions unless new image or bootarg evidence contradicts this entry. Do not treat QEMU vmfd readiness as the primary suspect unless a future A/B contradicts `20260429-165235`. For post-kernel VM1 slowness, add rate-limited per-op/per-device QEMU/sel4 request classification for the `3s` to `61s` interval and inspect GICv3 vGIC registered-IRQ/maintenance behavior; current counters prove there are real forwarded requests but not which virtio device or MMIO operation dominates. For pre-kernel VM1 load, the source-backed suspect is now specific: VM1 allocator-pool RAM uses 4 KiB touch/cache-maintenance chunks while VM0 fixed `untyped_mmios` RAM uses 2 MiB chunks, predicting about `10829/22 = 492x` more touch/clean iterations for the same 44,352,000 byte kernel and matching the measured `472x` VM1/VM0 timing gap. The user's 2024 RPi4 recollection is backed by old libsel4vm commits `ad99b3a`/`9d3ac10` plus app commit `c47cda6`: RPi4 explicitly used a `guest_large_pages` libsel4vm contract and set `vm1.guest_large_pages = true` even in the VMSWIOTLB-era app shape. Current `projects/sel4_projects_libs` instead has `c89de50`, which queries CAmkES page-size metadata and falls back to 4 KiB; current Orin VM1 does not make that query resolve to 2 MiB for VMSWIOTLB allocator-pool RAM. An RPi4 carry-over audit found dataport frame-array mapping, BAR sizing, reservation-map hardening, vCPU naming, and level IRQ semantics already present or evolved in current branch; guest-RAM large-page semantics is the main missing/superseded-badly item. Restore that old libsel4vm/app-level semantic or make the automatic page-size path cover this RAM shape; do not fix it by globally changing CAmkES simple-pool metadata. The separate ARM image-loader cache-maintenance bug was fixed in `projects/sel4_projects_libs` commit `7cf1a26`, so future timing should not rely on the old partial `0..4K` clean behavior for 2 MiB VM0 chunks. Treat seL4 kernel IPI slowness as low priority for this app unless affinity evidence changes: the generated image is SMP-capable, but VM0/VM1/VMM TCBs and one-vCPU guest TCBs are on affinity 0.
- Updated: 2026-04-29
